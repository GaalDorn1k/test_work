...
def main():
    ...
    def average_hours_per_day(worker):         
        amount_days = len(data_date[data_date['Исполнитель'] == worker].groupby(data_date['Дата']))
    ...
    def average_hours_for_task(worker):         
        amount_tasks = len(data_date[data_date['Исполнитель'] == worker].groupby(data_date['Задача']))
    ...
    def mean_deviation_time(worker):            
        worker = (data_date.loc[data_date['Исполнитель'] == worker])
    ...
    work_time = data_date['Часы'].groupby(data_date['Исполнитель']).sum()                                         

    # time for each task
    tasks_time = data_date['Часы'].groupby(data_date['Задача']).sum()                                           
    ...
    delta_time = data_time.loc[:,'Оценка'] - tasks_time
    delta_time.index = data_time['Задача']


    # Project time
    project_time = int(data_date.loc[ :, ['Часы'] ].sum())
    ...
    # workers dates
    workers_dates = data_date['Дата'].groupby(data_date['Исполнитель']).apply(pd.DataFrame)


    # all dates
    dates = data_date['Дата']
    ...
    for i in range(len(data_price['Исполнитель'])):
        costs += work_time[i] * data_price.loc[i]['Ставка']
        profit = income - costs
        profitability = (profit * 100) / income
    ...
    d = {'average_hours_per_day':   [average_hours_per_day(x)   for x in data_price['Исполнитель']],
        'average_hours_for_task':  [average_hours_for_task(x)  for x in data_price['Исполнитель']],
        'mean_deviation_time':     [mean_deviation_time(x)     for x in data_price['Исполнитель']],
        'absence_days':            [absence_days(x)            for x in data_price['Исполнитель']]}
    ...
    data_output.index = data_price['Исполнитель']
